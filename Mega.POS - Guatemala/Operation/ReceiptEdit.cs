        ...
        private void dataGridView1_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (e.RowIndex != -1)
                {
                    if (dataGridView1.Columns[e.ColumnIndex].DataPropertyName == "Count")
                    {
                        if (Convert.ToDecimal(dataGridView1.Rows[e.RowIndex].Cells[e.ColumnIndex].Value) > Convert.ToDecimal(dataGridView1.Rows[e.RowIndex].Cells["DocCount"].Value))
                        {
                            throw new Exception("La cantidad ingresada no puede ser mayor a la cantidad recibida");
                        }
                        //decimal c = Convert.ToDecimal(dataGridView1.Rows[e.RowIndex].Cells[e.ColumnIndex].Value);
