...
que le ofrece el sistema. Si el error persiste cierre la aplicación y ejecútela nuevamente por favor.";
        ...
        private void ConfirmSale()
        {
            ...
            if (SaleType != OperationType.ConsignationReturn && saleType != OperationType.Consignation)
            {
                ...
                if (oInvOperations.FoliosAl75porciento)
                    DialogHelper.ShowWarningInfo(this, "Ya se utilizo el 75% de la numeracion de facturacion");
            }
        ...
        {
            if (e.ReportPath == "FiscalReport")
            {
                var dsFiscal = new ReportDataSource("FiscalData")
                {
                    Value =
                        dc.vwResolutionFiscalTicket.Where(
                            o => o.IdOperation == currentItem.Id).Select(o => o)
                };
                e.DataSources.Add(dsFiscal);
            }
...
*/ 
        ...
        private void DoPayButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //Shop oShop = ApplicationHelper.GetCurrentShop();
                ResolutionFiscal res = dc.ResolutionFiscal.Single(o => o.Active == true);
                if (res.RangeFinal == Convert.ToInt32(oFShop.Folio))
                {
                    DialogHelper.ShowError(this, "La numeracion de facturacion a llegado a su limite, registre una nueva resolucion fiscal");
                    return;
                }
...
Si el error persiste cierre la aplicación y ejecútela nuevamente por favor.";
        ...
        private void FindSaleButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    var sale = dc.Invoices.Where(inv =>
                        (inv.Operation.IdType == (int)SaleType) && inv.Operation.IdStatus != (int)OperationStatus.NotConfirmed && inv.Folio == OperationConsecutiveTextBox.Text).SingleOrDefault().Operation;
