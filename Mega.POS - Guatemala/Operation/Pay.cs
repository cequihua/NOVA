...
namespace Mega.POS.Operation
{
    public partial class Pay : Form
    {
        ...
        public Pay(AdminDataContext dc, Common.Operation operation)
        {
            this.dc = dc;
        ...
        private string GetCurrentMonthPayrollAmount()
        {
            var pays = dc.Operation_Pays.Where(o => o.IdType == (int) MovementType.Payroll &&
                                                o.Operation.IdStatus == (int) OperationStatus.Confirmed &&
                                                o.Operation.IdDim == currentOperation.IdDim &&
                                                SqlMethods.DateDiffMonth(o.Operation.ModifiedDate, DateTime.Today) == 0); 

            return pays.Count() == 0 ? "0" : pays.Sum(o => o.OperationAmount).ToString("N");
        }
        ...
        private void LoadCurrencyCombo()
        {
            ...
            var curr = DataHelper.GetUDCCurrenciesByComboPay(dc, currentOperation.IdOperationCurrency);
        ...
        {
            DimNameLabel.Text = string.Format("{0} - {1}", currentOperation.IdDim, currentOperation.DimName);

            DimInfoLabel1.Text = string.Format("Puntos Anteriores: {0}, Fecha: {1}",
                                              currentOperation.Dim.PointAmount.ToString("N"),
                                              currentOperation.Dim.PointAmountDate.ToString("d"));

            DimInfoLabel2.Text = string.Format("En Compra: {0}, Puntos Finales: {1}",
                                               currentOperation.TotalPointOperation.HasValue
                                                   ? currentOperation.TotalPointOperation.Value.ToString("N")
                                                   : 0.ToString(),
                                               (currentOperation.Dim.PointAmount + currentOperation.TotalPointOperation ??
                                                0).ToString("N"));

            CreditMaxTextBox.Text = currentOperation.Dim.MaxCreditAmount.ToString("N");
            CreditCurrentTextBox.Text = currentOperation.Dim.CreditAmount.ToString("N");
            CreditDateTextBox.Text = currentOperation.Dim.CreditAmountDate.ToString("d");

            AmountOperationLabel.Text = currentOperation.OperationAmount.ToString("N");
            CurrencyOperationLabel.Text = currentOperation.OperationCurrencyCode;

            if (currentOperation.Dim.IdType == (int)DimType.DIM)
            {
                tabPagePayroll.Enabled = false;
                tabPageCredit.Enabled = true; 
            }
            else
            {
                tabPagePayroll.Enabled = true;
                tabPageCredit.Enabled = false;

                PayrollMaxByMonthTextBox.Text = currentOperation.Shop.Company.MaxAmountPayroll.ToString("N");
                PayrollCurrentMonthAmountTextBox.Text = GetCurrentMonthPayrollAmount();
            }
        ...
        {
            var rows = currentOperation.Operation_Pays;
            payDataGridView.DataSource = rows.ToList();
        ...
        {
            dc.Operation_Pays.DeleteAllOnSubmit(currentOperation.Operation_Pays);
            dc.SubmitChanges();
        ...
        private void AddCashButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    currentOperation.Operation_Pays.Insert(0, new Operation_Pay
                    {
                        Id = Guid.NewGuid(),
                        IdOperation = currentOperation.Id,
                        IdType = (int)MovementType.SaleCash,
                        IdOperationCurrency = currentOperation.IdOperationCurrency,
            ...
            {
                Logger.Error("Error al agregar el Pago en Efectivo", ex);
        ...
        {
            decimal amountPayed = currentOperation.Operation_Pays.Sum(p => p.Amount);
            byPay = Math.Round(currentOperation.OperationAmount - amountPayed, 2);

            decimal maxDiffChangeRate = currentOperation.Shop.Company.MaxDiffAmountInChangeRate;
            ...
            AddCreditButton.Enabled = !allowConfirmPay;
            AddCurrencyPayButton.Enabled = !allowConfirmPay;
            AddPayrollButton.Enabled = !allowConfirmPay;

            PayAndConfirmButton.Enabled = allowConfirmPay;

            CashReceivedTextBox.Text = "0";
            CardAmountTextBox.Text = byPay.ToString("N");
            CreditAmountTextBox.Text = byPay.ToString("N");
            PayrollAmountTextBox.Text = byPay.ToString("N");
        ...
        private void CurrencyReceivedTextBox_Validating(object sender, CancelEventArgs e)
        {
            ...
            CurrencyChangeTextBox.Text = DataHelper.IsActiveRoundByFive(dc, currentOperation.IdOperationCurrency)
        ...
        private void AddCardButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                currentOperation.Operation_Pays.Insert(0, new Operation_Pay
                {
                    Id = Guid.NewGuid(),
                    IdOperation = currentOperation.Id,
                    IdType = (int)MovementType.SaleCard,
                    IdOperationCurrency = currentOperation.IdOperationCurrency,
                    ChangeRate = (decimal)1.0000,
            ...
            {
                Logger.Error("Error inesperado al Agregar el pago por Tarjeta", ex);
        ...
        private void CreditAmountTextBox_Validating(object sender, CancelEventArgs e)
        {
            decimal toPay;
            if (CreditAmountTextBox.Text.Trim() == string.Empty || !decimal.TryParse(CreditAmountTextBox.Text, out toPay))
            {
                DialogHelper.ShowError(this, "El valor [Crédito] es requerido y debe ser numérico");
                e.Cancel = true;
            }
        }
        ...
        private void AddCreditButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                decimal toPay = decimal.Parse(CreditAmountTextBox.Text);

                if (decimal.Parse(CreditAmountTextBox.Text) + decimal.Parse(CreditCurrentTextBox.Text) > decimal.Parse(CreditMaxTextBox.Text))
                {
                    DialogHelper.ShowWarningInfo(this, "Si se agrega este pago a Crédito se sobrepasaría el límite Máximo otorgado al Dim/Cliente");
                    return;
                }
                ...
                currentOperation.Operation_Pays.Insert(0, new Operation_Pay
                {
                    Id = Guid.NewGuid(),
                    IdOperation = currentOperation.Id,
                    IdType = (int)MovementType.SaleCredit,
                    IdOperationCurrency = currentOperation.IdOperationCurrency,
                    ChangeRate = (decimal)1.0000,
                    OperationAmount = toPay,
                    Amount = toPay
            ...
            {
                Logger.Error("Error al agregar un pago de venta a credito", ex);
                DialogHelper.ShowError(this, "Error inesperado al ejecutar el pago a Crédito", ex);
            }
        }
        ...
        private void Pay_KeyDown(object sender, KeyEventArgs e)
        {
            switch (e.KeyCode)
            {
                ...
                case Keys.F6:
                    ...
                    break;
                case Keys.F7:
                    payTabControl.SelectTab(tabPageCredit);
                    break;
                case Keys.F8:
                    payTabControl.SelectTab(tabPageDivisa);
                    break;
                case Keys.F9:
                    payTabControl.SelectTab(tabPagePayroll);
                    break;
            }
        }
        ...
        private void UpdateCurrencyPayValues()
        {
            ...
            else
            {
                ...
                decimal changeRate = DataHelper.GetChangeRate(dc, currency, currentOperation.Shop.IdCurrency);
        ...
        private void AddCurrencyPayButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                else
                {
                    ...
                    decimal cash = DataHelper.GetCurrentCash(dc, Convert.ToInt32(currentOperation.IdOperationCurrency), new Guid(Properties.Settings.Default.CurrentCashier));
                    ...
                    currentOperation.Operation_Pays.Insert(0, new Operation_Pay
                    {
                        Id = Guid.NewGuid(),
                        IdOperation = currentOperation.Id,
                        IdType = (int)MovementType.SaleCurrency,
        ...
        private void AddPayrollButton_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                decimal toPay = decimal.Parse(PayrollAmountTextBox.Text);

                if (decimal.Parse(PayrollAmountTextBox.Text) + decimal.Parse(PayrollCurrentMonthAmountTextBox.Text) > decimal.Parse(PayrollMaxByMonthTextBox.Text))
                {
                    DialogHelper.ShowWarningInfo(this, "Si se agrega este pago por Nónima se sobrepasaría el límite Máximo Mensual otorgado a Empleados");
                    return;
                }
                ...
                currentOperation.Operation_Pays.Insert(0, new Operation_Pay
                {
                    Id = Guid.NewGuid(),
                    IdOperation = currentOperation.Id,
                    IdType = (int)MovementType.Payroll,
                    IdOperationCurrency = currentOperation.IdOperationCurrency,
                    ChangeRate = (decimal)1.0000,
                    OperationAmount = toPay,
                    Amount = toPay
            ...
            {
                Logger.Error("Error al agregar un pago de venta por Nómina", ex);
                DialogHelper.ShowError(this, "Error inesperado al ejecutar el pago por Nómina", ex);
            }
        }

        private void PayrollAmountTextBox_Validating(object sender, CancelEventArgs e)
        {
            decimal toPay;
            if (PayrollAmountTextBox.Text.Trim() == string.Empty || !decimal.TryParse(PayrollAmountTextBox.Text, out toPay))
            {
                DialogHelper.ShowError(this, "El valor [A pagar] por Nómina es requerido y debe ser numérico");
                e.Cancel = true;
            }
        }

        private void PayrollAmountTextBox_KeyPress(object sender, KeyPressEventArgs e)
        {
            if (e.KeyChar == (char)Keys.Return)
            {
                AddPayrollButton.Focus();
            }
        }
        ...
        private void payTabControl_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (payTabControl.SelectedTab == tabPageCredit)
            {
                //CashReceivedTextBox.Focus();
                ActiveControl = CreditAmountTextBox;
                CreditAmountTextBox.SelectAll();
            }
            ...
            if (payTabControl.SelectedTab == tabPagePayroll)
            {
                //CashReceivedTextBox.Focus();
                ActiveControl = PayrollAmountTextBox;
                PayrollAmountTextBox.SelectAll();
            }
        }
